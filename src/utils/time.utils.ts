...
export function parseTimeTo24Hour(value?: string): string | null {
  if (!value) {
    return null;
  }
  ...
  const [, hourPart, minutePart = "00", period] = match;
...
export function normalizeTimeTo24Hour(value?: string): string {
...
export function formatTimeTo12Hour(value?: string): string {
  if (!value)
    return "";
